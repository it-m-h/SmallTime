...
class time_group{
	...
	function __construct($_grpwahl){
		if($_grpwahl>=0){
			...
			for($x=1; $x< count($_groups->_array); $x++){ 
				...
				//echo count($tmpgrp);
				//print_r($tmpgrp);
				//echo " / : ".$_groups->_array[$x][2];
				//echo "<hr>"; 
				//if (!count($tmpgrp)==1) $tmpgrp = $_groups->_array[$x][2];
				//foreach($tmpUID as $UID){
				//	$tmpgrp[] = $UID; 
				//}
				//echo count($tmpgrp);
				//echo " / : ";
				...
				foreach($tmpgrp as $ma){
					...
					$u=0;
					//echo "<br> anzahl: ". count($this->_array[$x][5][$y]);
					// Anzahl Stempelzeiten
					$this->_array[6][$x][$y] = count($this->_array[5][$x][$y]);
					//print_r($this->_array[$x][5][$y]);
					$y++;
				}
				//$this->_array[$x][1] = $_groups->_array[$x][2]; 
				//echo "<hr color=red>";
			}	
			//echo "<hr>";
			//$zeig = new time_show($_groups->_array);
			//$zeig = new time_show($_users->_array);
			//$zeig = new time_show($this->_array);
			//echo "<hr>";		
		}
	}
	function __destruct(){
	...
	function get_usergroup($userid){
        ...
        foreach($_groups as $_group){
                //echo "Gruppe : " . $_group ." - <br>";
				$_group = explode(";", $_group);
				//echo $_group[2];
				//echo "Vergleiche : " . $_group[2]. " mit " . $userid . "<br>";
				$_group[2] = explode(",", $_group[2]);
                foreach($_group[2] as $users){
					if($users == $userid and $x>0)return $x;
				}
				//if(strpos($_group[2],$userid, 0) and $x>0) return $x;
                $x++; 
        }
	}
	function get_timestamps($_ordnerpfad){
		...
		//if (count($_timeTable))	$zeig = new time_show($_timeTable);
		//echo "<br>----";
		//echo $_ordnerpfad;
		//echo "<br><br>";
		
		//$this->_array[$x][6][] = "xx";
		//echo $_ordnerpfad. " - ". $_timeTable[count($_timeTable)-1];
		//echo "<br>";
			...
			foreach($_timeTable as $_time){
                ...
                if($_time){
					...
					if ($_w_jahr==$_w_jahr_t && $_w_monat==$_w_monat_t&&$_w_tag==$_w_tag_t){
						...
						//echo "<br>". $_w_stunde_t . ":" . $_w_minute_t . "<br>";
					}
				}else{
