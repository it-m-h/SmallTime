...
//echo "<br>File: $_file / submit : $_submit<br>";
if($_file && $_submit){
	//if ($_POST['uploadedfile'] && $_POST['submit']){
...
if(file_exists($_file)){
	$_timelist = array();
	$_show=1;
	//$_w_jahr_t         = date("Y", time());
	//$_w_monat_t         = date("n", time());
	//$_w_tag_t                = date("j", time());
	//$_w_stunde_t        = date("H", time());
	//$_w_minute_t         = date("i", time());
	//$_w_sekunde_t = date("s", time());
	if($_show){
