...
echo "<tr><td>";
echo "Filter: 
<input type='text' name='Filter' id='Filter' style='width: 95%; margin-bottom:10px; background-color: #b5d1bb;'>";
echo "</td></tr>";
echo "</table>";
...
unset($_benutzer[0]);
$i = 1;
$scrollAbUserAnzahl = 9;

if (count($_benutzer) >= $scrollAbUserAnzahl) {
	echo "<div id='ScrollDiv'>";
}

echo "<table width=100% border=0 cellpadding=5 cellspacing=1 id='UserList'>";
	...
	$("#Filter").keyup(function(){
		...
		$("#UserList tr").each(function(){
			if (founded) {
				$(this).show();
				founded = false;
			}else if ($(this).text().search(new RegExp(filter, "i")) < 0 && founded == false) {
				$(this).fadeOut();
			} else {
				$(this).show();
				count++;
		...
		$("#filter-count").text("Anzahl Mitarbeiter ="+count);
	});
});
</script>