...
for ($z=1; $z< count($_monat->_MonatsArray); $z++){
	//$_monat->_MonatsArray[$z][13] = 1;
	//$_std = number_format($_monat->_MonatsArray[$z][13], 0, '.', '').":";
	//$_temp = $_monat->_MonatsArray[$z][13] - number_format($_monat->_MonatsArray[$z][13], 0, '.', '');
	//$_tmp = $_tmp * 100 / 60;
	//$_tmp = number_format($_tmp, 0, '.', '');
	//$_std = $_std.$_tmp;
	//echo $_temp. "<hr>";
	
	
	//$_monat->_MonatsArray[$z][13] = $_std; 
	
	//echo $_monat->_MonatsArray[$z][13]. "<hr>";
}
}
?>
<table width=100% border=0 cellpadding=3 cellspacing=1>
	<tr>
		<td COLSPAN=3 class=td_background_top width=70 align=center>Datum</td>
		<td COLSPAN=<?php echo $t; ?> class=td_background_top width=550 align = left>Zeiten</td>
...
for ($z=1; $z< count($_monat->_MonatsArray); $z++){
	...
	//-------------------------------------------------------------------------
	echo "	<tr>\n";
	echo "		<td ". $_monat->_MonatsArray[$z][30]." align=center>". $_monat->_MonatsArray[$z][1]."</td>\n";
	...
	//-------------------------------------------------------------------------
	if($_monat->_MonatsArray[$z][11]%2==1) $tmp = $tmp . " - <font color=blue>Zeit fehlt!</font>";
	echo $tmp;
	echo "</td>\n";
	//-------------------------------------------------------------------------
	...
	if($_settings->_array[18][1]=="0" && $_monat->_MonatsArray[$z][13]>0){
		echo number_format($_monat->_MonatsArray[$z][13], 2, '.', '');
	}elseif($_settings->_array[18][1]=="1" && $_monat->_MonatsArray[$z][13]>0){
		//echo dec2time($_monat->_MonatsArray[$z][13], "%hh %imin");
		echo dec2time($_monat->_MonatsArray[$z][13], "%hh %imin");
	}
	echo "</td>\n";
	//-------------------------------------------------------------------------
	...
	//-------------------------------------------------------------------------
	echo "		<td ". $_monat->_MonatsArray[$z][30]." align=center>";
	if($_settings->_array[18][1]=="0"){
		if($_monat->_MonatsArray[$z][20]>0){
			echo number_format($_monat->_MonatsArray[$z][20], 2, '.', '');
		}elseif($_monat->_MonatsArray[$z][20]<0){
			echo "<font color=#b30000>". number_format($_monat->_MonatsArray[$z][20], 2, '.', ''). "</font>";
		}
	}else{
		if($_monat->_MonatsArray[$z][20]>0){
			//echo number_format($_monat->_MonatsArray[$z][20], 2, '.', '');
			echo dec2time($_monat->_MonatsArray[$z][20], "%hh %imin");
		}elseif($_monat->_MonatsArray[$z][20]<0){
			echo "<font color=#b30000>". dec2time($_monat->_MonatsArray[$z][20], "%hh %imin"). "</font>";
		}
	}
	echo "</td>\n";
	//-------------------------------------------------------------------------
...
?>
	<tr>
		...
		<td COLSPAN=<?php echo $t; ?> class=td_background_info width=550 align = left>Sollstunden : <?php
		...
		if($_monat->_SummeSaldoProMonat>0){
			...
			echo "<font color=#b30000>". number_format($_monat->_SummeSaldoProMonat, 2, '.', ''). "</font>";
		}
		?></td>
		<td COLSPAN=<?php echo $a; ?> class=td_background_info width=50 align=center><?php echo $_monat->_SummeAbsenzProMonat?></td>
		<?php if($_settings->_array[18][1]) echo "<td class=td_background_info width=16 align=center> </td>"; ?>
	</tr>
</table>