...
if($_template->_bootstrap){
	...
	foreach($monate as $_month){
		...
		if($i==$_time->_monat){			
			echo '		<li class="active"><a id="Monat" title="Monat '.$_month.'" href="?action=show_time&timestamp='.$_timestamp.'">'.$_month.'</a></li>';
		}else{
			echo '		<li ><a id="MonatAktiv" title="Monat '.$_month.'" href="?action=show_time&timestamp='.$_timestamp.'">'.$_month.'</a></li>';
		}
	$i++;
	}
	echo '	</ul>';
